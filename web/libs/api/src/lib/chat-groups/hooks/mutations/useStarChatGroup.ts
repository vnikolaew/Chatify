...
const starChatGroup = async ({ chatGroupId }: StarChatGroup) => {
   const { status, data } = await chatGroupsClient.post(
      `starred/${chatGroupId}`,
      {
         headers: {},
      }
...
export const useStarChatGroup = () => {
   ...
   return useMutation<any, Error, StarChatGroup, any>({
      mutationFn: starChatGroup,
      onError: console.error,
      onSuccess: async (_, { chatGroupId }) => {
         const starredChatGroup = client.getQueryData<ChatGroupDetailsEntry>([
            `chat-group`,
            chatGroupId,
         ]);
