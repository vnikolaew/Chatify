...
const getStarredChatGroups = async () => {
   const { status, data } =
      await chatGroupsClient.get<ChatGroupListApiResponse>(`starred`, {
         headers: {},
      });
...
export const useGetStarredChatGroups = (
   options?:
      | (Omit<
           UseQueryOptions<ChatGroup[], Error, ChatGroup[], any>,
   ...
   return useQuery<ChatGroup[], Error, ChatGroup[], any>({
      queryKey: STARRED_CHAT_GROUPS_KEY,
