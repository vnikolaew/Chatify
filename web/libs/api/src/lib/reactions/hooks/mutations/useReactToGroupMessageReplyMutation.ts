...
export const useReactToGroupMessageReplyMutation = () => {
   ...
   return useMutation<any, Error, ReactToGroupMessageModel, any>(reactToGroupMessageReply, {
      ...
      onSuccess: (id, { messageId, groupId, reactionType }) => {
         console.log(
            "Successfully reacted to chat group message reply. Reaction Id: " + id,
         );
         ...
         client.setQueryData<InfiniteData<CursorPaged<ChatMessageReply>>>(
            GET_PAGINATED_GROUP_MESSAGE_REPLIES_KEY(messageId), (old) =>
               produce(old, (replies: InfiniteData<CursorPaged<ChatMessageReply>>) => {
