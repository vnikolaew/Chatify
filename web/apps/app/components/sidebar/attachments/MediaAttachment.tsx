      ...
      >
         ...
         <div className={`flex flex-col gap-0 items-start`}>
            ...
            <p className={`text-default-500 w-full text-[.7rem]`}>
               {t.rich(`AttachmentDescription`, {name: attachment.userId === meId ? ` ${attachment.username} (you)` : attachment.username, date: moment(new Date(attachment.createdAt)).format("DD/MM/YYYY"), nameBold: (chunks) => <b className={`text-default-300`}>{chunks}</b>})}
            </p>
         </div>
      </div>
   );
